...
const BandInfoParser_js_1 = __importDefault(require("./BandInfoParser.js"));
const DiscographyParser_js_1 = __importDefault(require("./DiscographyParser.js"));
const LabelArtistsParser_js_1 = __importDefault(require("./LabelArtistsParser.js"));
class BandAPI extends BaseAPIWithImageSupport_js_1.default {
    async getDiscography(params) {
        ...
        const html = await this.fetch((0, Parse_js_1.normalizeUrl)('music', params.bandUrl));
        return DiscographyParser_js_1.default.parseDiscography(html, opts);
    }
    async getInfo(params) {
        ...
        const result = BandInfoParser_js_1.default.parseInfo(html, opts);
        ...
        const info = BandInfoParser_js_1.default.parseInfo(musicHtml, opts);
        ...
        if (discogItems[0]) {
            ...
            if (url) {
                const html = await this.fetch(url);
                const info = BandInfoParser_js_1.default.parseInfo(html, opts);
                BandAPI.fillInfo(result, info);
            }
        }
        if (result.url === null) {
    ...
    async getLabelArtists(params) {
        ...
        const html = await this.fetch((0, Parse_js_1.normalizeUrl)('artists', params.labelUrl));
        return LabelArtistsParser_js_1.default.parseLabelArtists(html, opts);
    }
    /**
     * @internal
     */
    static getUrl(artistOrLabelUrl, path, labelId) {
        let url = path ? (0, Parse_js_1.normalizeUrl)(path, artistOrLabelUrl) : artistOrLabelUrl;
        if (labelId) {
...
exports.default = BandAPI;
class LimiterBandAPI extends BandAPI {
    constructor(params) {
        super(params);
...
exports.LimiterBandAPI = LimiterBandAPI;
